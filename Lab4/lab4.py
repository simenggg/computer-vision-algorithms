...
def harris_corners(img, window_size=3, k=0.04):
    ...
    A = convolve(grad_x_2, window) 
    B = convolve(grad_xy, window) 
    C = convolve(grad_y_2, window)
...
def naive_descriptor(patch):
    ...
    mean = np.mean(patch)
    std = np.std(patch)
    normalized_patch = (patch - mean) / (std + 0.0001)
...
def simple_sift(patch):
    ...
    """ Your code starts here """
    grad_x = filters.sobel_v(patch)
    grad_y = filters.sobel_h(patch)
    grad_magnitude = np.sqrt(grad_x ** 2 + grad_y ** 2)
    grad_orientation = np.arctan2(grad_y, grad_x) * (180 / np.pi) % 360

    for i in range(patch.shape[0]):
        for j in range(patch.shape[1]):
            # which cell does the pixel belong to  
            cell_x = j // 4
            cell_y = i // 4

            # which orientation bin does the pixel belong to 
            orientation_bin = int(grad_orientation[i, j] // 45) % 8
            histogram[cell_y, cell_x, orientation_bin] += grad_magnitude[i, j] * weights[i, j]

    # 128 dimensions and normalized to unit length
...
def top_k_matches(desc1, desc2, k=2):
    ...
    """ Your code starts here """
    pairwise_dis = cdist(desc1, desc2, metric='euclidean')
    for i in range (desc1.shape[0]):
        nearest_indices = np.argsort(pairwise_dis[i])[:k]
        nearest_distances = pairwise_dis[i][nearest_indices]
        match_pairs.append((i, list(zip(nearest_indices, nearest_distances))))
...
def ratio_test_match(desc1, desc2, match_threshold):
    ...
    """ Your code starts here """
    for i, matches in top_2_matches:
        idx1, dis1 = matches[0]
        idx2, dis2 = matches[1]
        if dis1 / dis2 < match_threshold:
            match_pairs.append((i, idx1))
...
def compute_cv2_descriptor(im, method=cv2.SIFT_create()):
    ...
    Do note that the keypoints coordinate is (col, row)-(x,y) in OpenCV. We have changed it to (row,col)-(y,x) for you. (Consistent with our coordinate choice)
    '''
...
def compute_homography(src, dst):
    ...
    src_subtracted = src - src_mean
    dst_subtracted = dst - dst_mean

    src_average_dist = np.mean(np.sqrt(np.sum(src_subtracted ** 2, axis=1)))
    dst_average_dist = np.mean(np.sqrt(np.sum(dst_subtracted ** 2, axis=1)))

    src_scale = np.sqrt(2) / src_average_dist
    dst_scale = np.sqrt(2) / dst_average_dist

    normalization_matrix_src = np.array([[src_scale, 0, src_scale * -src_mean[0]],
                                         [0, src_scale, src_scale * -src_mean[1]],
                                         [0, 0, 1]])

    normalization_matrix_dst = np.array([[dst_scale, 0, dst_scale * -dst_mean[0]],
                                         [0, dst_scale, dst_scale * -dst_mean[1]],
                                         [0, 0, 1]])
    src_normalized = transform_homography(src, normalization_matrix_src)
    dst_normalized = transform_homography(dst, normalization_matrix_dst)

    # DLT algorithm
    A = []
    for i in range(src_normalized.shape[0]):
        # for every point in the src, compute the 2 by 9 matrix for A and stack them tgt 
    ...
    h_matrix = np.linalg.inv(normalization_matrix_dst).dot(h_normalized).dot(normalization_matrix_src)
...
def ransac_homography(keypoints1, keypoints2, matches, sampling_ratio=0.5, n_iters=500, delta=20):
    ...
    Args:
        ...
        delta: the inlier distance threshold

    Returns:
        H: a robust estimation of homography from keypoints2 to
        keypoints1
    """
    N = matches.shape[0]
    ...
    for i in range(n_iters):
        ...
        src_samples = matched1_unpad[sample_indices]
        dst_samples = matched2_unpad[sample_indices]
        H = compute_homography(src_samples, dst_samples)
        transformed_points = transform_homography(matched1_unpad, H)
        distances = np.linalg.norm(transformed_points - matched2_unpad, axis=1)
        curr_inliers = np.sum(distances < delta)
        if curr_inliers > n_inliers:
            n_inliers = curr_inliers
            max_inliers = distances < delta
            # max_inliers is a boolean array indicating which matches are inliers
...
def create_mirror_descriptors(img):
    '''
    Return the output for compute_cv2_descriptor (which you can find in lab4.py)
    ...
    mir_descs = np.zeros_like(descs)
    for i in range(descs.shape[0]):
        # for every single descriptor in descs, find the mir_descs 
        mir_descs[i] = shift_sift_descriptor(descs[i])
    """ Your code ends here """
    
    return kps, descs, angles, sizes, mir_descs
...
def match_mirror_descriptors(descs, mirror_descs, threshold = 0.7):
    ...
    for i, matches in three_matches:  
        # remove the match that comes from the keypoint itself
        filtered_matches = [(idx, dis) for idx, dis in matches if idx != i]
        if len(filtered_matches) < 2:
            continue

        idx1, dis1 = filtered_matches[0]
        idx2, dis2 = filtered_matches[1]
        if dis1 / dis2 < threshold:
            match_result.append((i, idx1))
...
def find_symmetry_lines(matches, kps):
    ...
    for match in matches:
        ...
        pj = kps[idx2]
        mid = midpoint(pi, pj)
        angle = angle_with_x_axis(pi, pj)
        #theta = angle + np.pi / 2
        #theta = theta % (2 * np.pi)
        # due to coordinate convention 
        theta = angle
        rho = mid[1] * np.cos(theta) + mid[0] * np.sin(theta)
        rhos.append(rho)
        thetas.append(theta)    
...
def hough_vote_mirror(matches, kps, im_shape, window=1, threshold=0.5, num_lines=1):
    ...
    rho_values = np.arange(-diag_len, diag_len + 1, 1)
    theta_values = np.arange(0, 2 * np.pi, np.radians(1))
    hspace = np.zeros((len(rho_values), len(theta_values)), dtype=np.int32)

    for i in range(len(rhos)):
        # for every pair of (rho, theta), which is every pair of matches 
        rho = rhos[i]
        theta = thetas[i]
    ...
    res = find_peak_params(hspace, [rho_values, theta_values], window_size=window, threshold=threshold)

    peak_values = res[0][:num_lines]
    rho_values = res[1][:num_lines]
    theta_values = res[2][:num_lines]
    """ Your code ends here """
